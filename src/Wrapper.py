...
def test(mapped_data):

    for i in np.unique(mapped_data['Link']):
        model, spectrum = np.array(list(mapped_data)[:len(mapped_data['Link'])])[(np.array(mapped_data['Link']) == i)]
        print(mapped_data[model], mapped_data[spectrum])
        print()
